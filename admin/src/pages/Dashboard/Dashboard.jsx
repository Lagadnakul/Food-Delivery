...
import { RevenueChart, OrdersBarChart, CategoryPieChart } from '../../components/Charts/Charts'
...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    setLoading(true)
    try {
      const [foodResult, ordersResult] = await Promise.all([listFood(), listOrders()])

      const items = foodResult.success ? foodResult.data : []
      const orders = ordersResult.success ? ordersResult.data : []

      const totalRevenue = orders.reduce((sum, o) => sum + (o.amount || 0), 0)
      const pendingOrders = orders.filter(o => o.status !== 'Delivered').length

      setStats({ totalOrders: orders.length, totalRevenue, totalItems: items.length, pendingOrders })
      setAllOrders(orders)
      setFoodItems(items)
  ...
  const statsCards = [
    ...
    {
      title: 'Food Items', value: stats.totalItems,
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'Food Processing': return 'bg-yellow-100 text-yellow-700'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div>
          <CategoryPieChart foodItems={foodItems} />
        </div>
      </div>

      <div>
        <OrdersBarChart orders={allOrders} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {recentOrders.length > 0 ? (
            <div className="space-y-4">
              {recentOrders.map((order, i) => (
                <div key={order._id || i} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          ...
          <div className="mt-6 p-4 bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl text-white">
            ...
            <p className="text-sm text-slate-300 mb-4">{stats.totalOrders} total orders processed!</p>
            <div className="h-2 bg-white/20 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-green-400 to-emerald-400 rounded-full" style={{ width: `${Math.min(100, stats.totalOrders * 10)}%` }} />
            </div>
          </div>
        </div>
      </div>
